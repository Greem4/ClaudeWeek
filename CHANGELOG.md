...
- **В переключателе Claude / Codex стоят знаки самих сервисов.** Раньше это
  ...
  Codex, крашенные под тему как шаблонные, и сведены в общую рамку: выбранный
  сервис виден по подложке и чёрточке под знаком. Картинки лежат в ресурсном
  бандле таргета приложения (`Sources/ClaudeWeekApp/Assets`), и `make-app.sh`
