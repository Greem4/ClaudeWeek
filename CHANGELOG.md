...
- **Кнопка «Начать счёт заново»** и строка **«Сейчас в ключе»** на вкладке
  ...
  заметить неоткуда.
- **Статистика окна в уведомлении.** К двум строкам баннера добавилась третья:
  с какого дня идёт окно лимита и на какую модель ушло больше всего («Неделя с
  СБ · больше всего Opus 62 %»). У пятичасовой сессии опора не день, а час
  начала, и разбивка для неё считается отдельно — по транскриптам ровно за эти
  пять часов, а не за неделю.
...
- **Картинка уведомления стала сводкой окна.** Вместо одной крупной формы —
  дуги у сессии и числа у недели — теперь подпись окна, процент, полоса
  расхода и главная модель под ней. Причина: развёрнутый баннер показывает
  вложение большим, и всё это место занимала цифра, уже написанная в первой
  строке текста.
