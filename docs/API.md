...
# Официальный источник Codex: `codex app-server`

**Проверено:** 2026-08-30, Codex CLI 0.151.0. В отличие от эндпоинта Claude,
это [публично документированный интерфейс](https://learn.chatgpt.com/docs/app-server).
ClaudeWeek запускает установленный `codex app-server --stdio`; авторизацией
занимается сам Codex, его токены приложение не читает.

## Обмен

Протокол — JSONL поверх stdin/stdout. Сначала отправляется `initialize`, после
его ответа — уведомление `initialized`. Только затем можно спрашивать данные:
...
Каждое обновление — отдельный короткоживущий процесс с таймаутом 20 секунд.
Между удачными запросами действует минутный троттлинг; после ошибок — та же
пауза 1 → 2 → 4 → 8 → 15 минут и предел устаревания 15 минут, что у Claude.

CLI ищется в `CODEX_EXECUTABLE`, `PATH`, обычных каталогах npm/Homebrew и
внутри `/Applications/Codex.app`. Если он не найден или аккаунт не
авторизован, наружу идёт честная `UsageError`, а не нулевой процент.

## Лимиты

`account/rateLimits/read` возвращает совместимый одиночный `rateLimits` и,
если лимитов несколько, `rateLimitsByLimitId`:
...
Если сервер вернул `rateLimitsByLimitId`, сначала выбирается bucket с ключом
`codex` (либо с внутренним `limitId == "codex"`). Модельные bucket рядом с ним
не считаются общим лимитом аккаунта; одиночный `rateLimits` служит только
запасным вариантом для старых версий app-server.

`usedPercent` — расход 0…100, `windowDurationMins` — настоящая длина окна,
`resetsAt` — Unix timestamp в секундах. Позиция сама по себе не определяет
назначение окна: недельным считается `secondary` либо `primary` длительностью
не меньше шести суток; короткий действующий `primary` попадает в отдельную
строку панели. Окно без времени сброса нельзя показать как действующее, но оно
не мешает прочитать остальные лимиты ответа.

## Дневная форма

`account/usage/read` отдельно отдаёт необязательные дневные токенные бакеты:

```json
{"dailyUsageBuckets":[{"startDate":"2026-06-18","tokens":12345}]}
```

Они задают относительную форму расхода по дням. Накопительная форма
масштабируется так, чтобы последнее известное значение совпало с точным
`usedPercent` недельного окна. Поэтому итог недели точный
(`isEstimate == false`), а дневные полосы помечены в модели как оценка
(`shapeIsEstimate == true`). Разбивки по моделям этот интерфейс не сообщает.

`startDate` — календарный день **тихоокеанской зоны** (зона биллинга OpenAI),
а не UTC и не местной зоны: проверено на живом ответе — обмен в 05:53 UTC попал
в бакет предыдущего числа. Окно недели живёт в зоне конфига, поэтому бакет
раскладывается по суткам окна пропорционально настоящему перекрытию во времени
(`[местная полночь дня, +сутки)` против границ суток окна), а не сопоставлением
дат. Без этого в первые местные сутки недельного окна единственный бакет
уезжал за его левую границу, и вся дневная форма пропадала — хотя точный
процент недели сервер уже сообщил. Если перекрытия всё же нет ни у одних
прошедших суток, точный недельный процент вешается на последние из них.

Снимок хранится отдельно в `~/.config/claude-week/codex-cache.json`; кеш Claude
