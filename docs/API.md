...
Второй вариант — свой токен, вставленный в настройках (вкладка «Доступ»,
`authSource: manual`). Он лежит в отдельной записи Keychain `ClaudeWeek-token`
и нужен, когда Claude Code стоит под другим пользователем или смотреть надо
чужой аккаунт. Запись Claude Code при этом не трогается: читать чужие креды
можно, писать в них — нет.
...
свежее нельзя. Старше двух минут — панель подписывает «данные 6 мин назад»
(`PanelModel.freshFor`). Старше пятнадцати — число отбрасывается совсем
(`OfficialProvider.staleLimit`): локальная оценка по сегодняшним транскриптам
честнее позавчерашнего официального процента, выданного за сегодняшний.
