...
долларов по моделям в ответе нет ни в каком виде. Поэтому окно «Расход по
моделям» (клик по цифрам процента в панели) считает разбивку по локальным
транскриптам и взвешивает её по ценам моделей — итог недели при этом остаётся
официальным, а деление между моделями помечено `≈`.
