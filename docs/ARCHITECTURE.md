...
сессия рисуется дугой, неделя красным числом, тем же языком, что значок в
строке меню. Цвет берётся по порогам самих уведомлений, а не по цветовым со
вкладки «Строка меню»: баннер о пробитом пороге со спокойным зелёным числом
противоречил бы поводу, по которому пришёл.
