...
- **Ряд дней целиком** — `appearance.showsPlan`. Выключенный убирает
  `DayBar`-ряд и ставит на его место `WeekRow` (`PopoverView.weekRow(_:)`):
  та же `LimitBar`, что у `SessionRow`, без плановой зоны. `panelLayout` в
  этом виде не смотрят — делить нечего.
