...
- **«Почему строк восемь и почему крайние короче»** → `WeekWindow.bounds(...)`:
  сутки календарные, а окно начинается в час сброса.
...
9. **Строки панели — календарные сутки местной зоны** (`WeekWindow.dayBounds`),
   а не интервалы «от сброса до сброса». Их семь только когда сброс попадает
   в полночь, иначе восемь: крайние обрезаны границами окна. Всё, что делит
   расход по строкам, спрашивает длину у `window.slotCount`, а не у
   `WeekWindow.daysInWeek` — та осталась длиной окна в сутках.
10. **План строки считается временем, а не номером строки**
   (`planPercent(forDay:anchor:)` зовёт `planPercent(at:)`). Иначе обрезанные
   сутки получили бы долю полных, а ряд не сошёлся бы к 100 % в момент сброса.
11. **Время в плане — рабочее, а не астрономическое** (`WorkHours`,
...
12. **Кривой конфиг чинится, а не отвергается** (`Config.validated()`):
   потерять строку меню из-за опечатки в JSON нельзя.
13. **Токен не логируется, не пишется в конфиг и не выводится в UI.**
