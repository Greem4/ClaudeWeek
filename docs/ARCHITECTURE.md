...
- **Клик по пункту строки меню обгоняет уведомление о смене стола.**
  ...
  `dropdown.isShown`, то есть по `panel.isVisible`, а честность этого признака
  держится на одном асинхронном событии — `activeSpaceDidChangeNotification`,
  по которому `handleSpaceChange` закрывает панель. Уведомление приходит,
  когда переход между столами уже завершился; клик, попавший в промежуток
  между началом перехода и уведомлением, видит `isVisible == true` (панель всё
  ещё висит на покинутом столе) и вместо показа вызывает `close()`. Со стороны
  это ровно «на одном столе открывается, на другом нет»: панель не появляется,
  помогает второй клик. Ловится тем чаще, чем быстрее переключают столы —
  свайпом или `⌃→`, не дожидаясь конца анимации, — и почти не ловится, если
  между переходом и кликом есть пауза.

  Чинить надо не таймингом, а признаком: спрашивать не «окно видимо», а
  «окно видно здесь». Публичный способ — `CGWindowListCopyWindowInfo` с
  `kCGWindowListOptionOnScreenOnly`: нет `panel.windowNumber` в списке —
  значит панель на чужом столе, и клик должен показывать её, а не закрывать.
  `isOnActiveSpace` для этого не годится (см. грабли выше).

  Рядом, в `DropdownPanel.show(from:)`, стоит `panel.makeKey()` — он нужен
  ради Esc, но у неактивного приложения makeKey тянет за собой ту же привязку
  к «своему» пространству, от которой уходит `orderOut` строкой выше. На
  подозрении как вторая причина мигания, но подтвердить это чтением кода
  нельзя — нужен прогон живьём с логом `windowNumber` и списка окон на экране
  до и после показа.
