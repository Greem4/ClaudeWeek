...
пропущенное после сна. Нашлась версия новее — `UpdateController.state`
переходит в `.available`, и это разом видно в трёх местах: строкой внизу
панели, названием пункта меню и на вкладке «О программе». Дальше только по
кнопке: `UpdateInstaller` (свой актор, чтобы `hdiutil` и копирование каталога
не вешали главный) качает образ, сверяет SHA256 с `SHA256SUMS.txt`, проверяет
подпись и версию внутри бандла и заменяет работающий одним `replaceItemAt`.
Перезапуск — отдельным шагом: `open -n` из отделённого процесса через секунду
после нашего выхода, иначе в строке меню на мгновение оказались бы два значка.
