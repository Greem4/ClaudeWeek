...
10. **Строк на панели всегда семь** (`WeekWindow.rowOrder(at:)`,
...
Какой это лимит, говорит третья строка текста и подпись на картинке
(`AlertArtwork`): «Неделя с СБ», «Сессия с 17:30». Картинка — сводка окна:
подпись, процент, полоса расхода и главная модель под ней. Разбивку за
пятичасовое окно ядро не знает — снимок считает её за неделю, — поэтому
`NotificationController` досчитывает её перед отправкой через
`ResolvingProvider.models(from:to:)`; один обход транскриптов на баннер, а
баннеров единицы в неделю.

Цвет картинки берётся по порогам самих уведомлений (`LimitState.forPercent`
