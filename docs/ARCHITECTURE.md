...
- **Клик по пункту строки меню обгоняет уведомление о смене стола.**
  `togglePanel` (`StatusItemController`) решает «открыть или закрыть» по
  `dropdown.isShown`, а тот держался на `panel.isVisible` — признаке, честном
  лишь потому, что `handleSpaceChange` закрывал панель на каждой смене стола.
  Уведомление `activeSpaceDidChangeNotification` приходит, когда переход уже
  завершился; клик, попавший в промежуток между переходом и уведомлением,
  видел `isVisible == true` (панель всё ещё висела на покинутом столе) и
  вместо показа вызывал `close()`. Со стороны это ровно «на одном столе
  открывается, на другом нет»: панель не появляется, помогает второй клик.
  Ловилось тем чаще, чем быстрее переключают столы — свайпом или `⌃→`, не
  дожидаясь конца анимации.

  Починено признаком, а не таймингом: `isShown` спрашивает не «окно видимо»,
  а «окно видно здесь» — `CGWindowListCopyWindowInfo` с
  `kCGWindowListOptionOnScreenOnly`, нет `panel.windowNumber` в списке, значит
  панель на чужом столе (`DropdownPanel.isOnActiveSpaceScreen`).
  `isOnActiveSpace` для этого не годится, см. грабли выше. Разрешений список
  не требует: за «Записью экрана» закрыты только имена окон, а мы читаем
  номера. Внутри `DropdownPanel` рядом живёт `isOpen` — это по-прежнему
  `isVisible`, и спрашивают его там, где вопрос другой: «есть ли что
  закрывать» (`close`, монитор кликов, `pinToAnchor`).

  Из той же гонки следует вторая половина починки: `handleSpaceChange` больше
  не закрывает панель без разбора, а только ту, которой на активном столе нет.
  Иначе запоздавшее уведомление гасило бы панель, открытую кликом сразу после
  свайпа, — через мгновение после появления.

  На подозрении остаётся `panel.makeKey()` в `DropdownPanel.show(from:)`: он
  нужен ради Esc, но у неактивного приложения makeKey тянет за собой ту же
  привязку к «своему» пространству, от которой уходит `orderOut` строкой выше.
  Подтвердить это чтением кода нельзя — нужен прогон живьём с логом
  `windowNumber` и списка окон на экране до и после показа.
