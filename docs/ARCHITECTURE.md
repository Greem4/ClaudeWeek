...
| `ModelsWindow.swift` | окно «Расход по моделям»: заводится при первом открытии, показывает ту же `PanelModel`, что и панель |
| `ModelsView.swift` | вёрстка этого окна: таблица моделей, доли, токены по видам, оговорка про оценку |
...
| `Screenshot.swift` | `--screenshot`: панель во всех темах, окно моделей в светлой и тёмной, иконка в PNG |
