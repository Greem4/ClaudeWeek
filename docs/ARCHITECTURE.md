...
`render()` — иконку строки меню. Ошибка → `model.apply(error:)`, причина уходит
в подсказку кружка источника: были данные — кружок желтеет и подсказка называет
ещё и их возраст, не было — гаснет в серый контур.

**Свежесть — одно правило на всех.** Решает `PanelModel.apply(_:at:)`: снимок
старше `freshFor` (2 минуты) переводит статус в `.stale("данные 6 мин назад")`.
Так подписывается и восстановленный из кеша снимок при старте, и официальное
число, отданное во время паузы после отказа. Отдельной пометки «из кеша» нет
намеренно: важен возраст, а не то, из какого файла его достали.
