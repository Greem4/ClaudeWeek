...
7. **Локальный расход считается только с `countFrom`** (`CountingState`,
   ...
   приходит из окна настроек, когда провайдер давно создан. Из **счёта**
   до-отсечный расход выброшен, но не из **формы**: `LocalUsage.costBeforeCount`
   помнит его отдельно, и `OfficialProvider.shapeByDay` раскладывает его по
   плану отрезка «начало окна → отсечка» (`carriedShares`). Итог недели сервер
   считает целиком, отсечки не зная, — без этого ряд стоял бы пустым
   (`— / 43 %` во всех сутках) при непустой неделе, а футер показывал бы свои
   проценты. Ровный темп в рабочие часы — та же догадка, по которой считается
   план: подённой правды об этом расходе нет, а сложенный горкой в одни сутки,
   он зажигал в них перерасход, которого не было.
...
11. **Строк на панели всегда семь** (`WeekWindow.rowOrder(at:)`,
   ...
   - `.reset` — ряд от суток сброса, как неделя катится. Показывается та
     ...
     (последняя, 100 %). Переезд не случается, когда у последних суток нет
     рабочих часов вовсе (сброс среди ночи: окно кончается в 02:00, план
     растёт с 11:00): их строка повторила бы предыдущую теми же 100 % —
     план в ней не набирается нисколько. Тогда день сброса остаётся в ряду
     вечерней половиной, а расход суток, оставшихся без строки, доливается
     в последнюю строку ряда (`UsageSnapshot.rows(at:)`): показать его больше
     негде, а разойтись с итогом в футере ряд не должен.
...
`make-app.sh` кладёт ресурсы SwiftPM (пока это один знак Claude для
переключателя аккаунтов) поштучно в `Contents/Resources`, а сам ресурсный бандл
`ClaudeWeek_ClaudeWeekApp.bundle` в `.app` не переносит, и код читает их через
`Bundle.main`. `Bundle.module` в приложении запрещён, и запрет проверяет сама
сборка: генерируемый SwiftPM аксессор ищет ресурсный бандл ровно в двух местах
— в корне `.app`, куда в бандле macOS ничего не кладут (всё живёт под
`Contents/`), и по абсолютному пути в `.build` той машины, где собирали. На
машине сборщика второй путь есть, и подмены не видно; в образе из релиза он
ведёт в `/Users/runner/work/…`, которого нет ни у кого. Так умирала 0.2.1:
обновление вставало, приложение падало `fatalError`'ом при первой отрисовке
панели, и человек оставался на прежней версии. Скрипт теперь сверяет, что
каждый ресурс лёг туда, откуда его возьмёт `Bundle.main`, — прежняя проверка
убеждалась лишь в том, что бандл есть в `.build`, а он был на месте всё то
время, пока приложение падало.
...
- **`Bundle.module` работает только у того, кто собирал.** Генерируемый
  SwiftPM аксессор ищет ресурсный бандл в корне `.app` и по абсолютному пути в
  `.build` машины сборки. У собиравшего второй путь есть, поэтому локально
  ресурс находится и подмены не видно; в образе из релиза он ведёт в
  `/Users/runner/work/…`, и `Bundle.module` роняет приложение `fatalError`'ом.
  Так умерла 0.2.1: обновление вставало, приложение падало через секунду после
  запуска, и человек оставался на прежней версии — а назавтра всё повторялось.
  Ресурсы читаются через `Bundle.main` из `Contents/Resources`, и `make-app.sh`
  на `Bundle.module` в коде приложения сборку не отдаёт.
