...
| Показать пример | шлёт такой же баннер, как настоящий, с первым недельным порогом вместо живого расхода; на историю сказанного не влияет |
| Недельный лимит | уведомлять ли о неделе и после скольких процентов — два порога, по умолчанию 80 и 95 |
| Пятичасовая сессия | то же для сессии, по умолчанию 75 и 95 |
...
Баннер macOS приходит, когда расход перешагнул заданный порог. Что в нём:

```
Недельный лимит — 84 %
Осталось 16 %. Сброс через 2 дн 4 ч (ПТ 16:00).
```

Заголовок называет лимит и живое число (оценка помечается знаком `≈`, как и в
панели), текст — остаток и момент сброса, а справа стоит то же кольцо, что в
строке меню, и того же цвета: он берётся по **цветовым** порогам, чтобы баннер
не спорил со значком. Исчерпанный лимит говорит об этом прямо: «Недельный
лимит исчерпан», «Пятичасовая сессия исчерпана» — и когда отпустит.
...
- у отладочного `swift run` уведомлений нет вовсе: macOS опознаёт программу по
  бандлу, которого там нет. Кнопка «Показать пример» в этом случае неактивна.
