...
ручная калибровка, обновление, пути к конфигу, кешу и логу с кнопками
«Открыть», а также «Сбросить настройки» — с подтверждением, и сброс не трогает
...
Скачивания и подмены сама не делает — только показывает: строкой внизу панели
(`↑ вышла версия 0.1.4` с кнопкой «Обновить»), пунктом в меню по правому щелчку
и строкой на вкладке «О программе». Программа в строке меню не должна
перезапускаться посреди работы без спроса.

Что происходит по кнопке «Обновить»:
...
После этого кнопка становится «Перезапустить» — до перезапуска в строке меню
живёт прежняя версия. Конфиг, кеш, калибровка и launchd-агент не трогаются.
