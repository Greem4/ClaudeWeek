...
или по Esc. Правый клик по иконке даёт обычное меню: «Обновить», «Запускать
при входе в систему», «Настройки…», «О программе», «Выйти».

Галочка «Запускать при входе в систему» правит того же launchd-агента, что
ставит `install.sh`, — файл `~/Library/LaunchAgents/com.greem4.claudeweek.plist`.
Включение и выключение начинают действовать со следующего входа в систему:
запущенное приложение галочка не гасит и второй копии не поднимает. У
отладочного `swift run` пункт неактивен — автозапускать там нечего.
