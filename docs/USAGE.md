...
справа — её же в процентах, со знаком `≈`. Заголовок меняется на «ЧЕМ
ПОТРАЧЕНО», чтобы ряд не читался как недельный со странными подписями, а под
строками стоит подпись: **≈ прикинуто на месте, по вашим транскриптам; сервер
сообщает только итог недели.**
