...
Запись Keychain второго дома Claude Code называет по-своему и это имя нигде не
публикует. Выводить его формулой значило бы закрепить догадку о чужой
внутренней детали: она сменится с обновлением CLI, и виджет молча покажет
цифры не того аккаунта. Поэтому имя не выводится, а **находится**: среди
записей Claude Code берётся та, чей `organizationUuid` совпал с `orgId`,
который назвал `claude auth status` для этого дома. Совпадений оказалось не
одно — виджет честно скажет «нет доступа», а не выберет наугад.
