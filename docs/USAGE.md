...
### Чем потрачена неделя

**Щелчок по самим цифрам процента** — справа в строке дня или в строке
сессии — открывает окно «Расход по моделям»: чем именно набран недельный
лимит. Строка на модель, от дорогой к дешёвой:

| Столбец | Что в нём |
|---|---|
| Доля | сколько процентов недельного расхода пришлось на эту модель |
| В лимите | та же доля, пересчитанная в проценты недельного лимита |
| Вход, Выход, Кеш | токены по видам; наведите на строку — увидите запись и чтение кеша отдельно и условную стоимость |
| Ответов | сколько ответов модели попало в окно |

В компактном виде клик по строке раскрывает неделю, а клик по цифрам в той же
строке открывает окно: действия разведены по местам, и попасть в цифры мышью
проще, чем промахнуться.
...
офлайн-оценка недели. Знак `≈` в столбце «В лимите» относится именно к этому:
итог недели над таблицей остаётся точным числом сервера, а его дележ между
моделями — правдоподобным.

Отсюда же два случая, когда окно пустое: транскриптов за это окно нет
(`~/.claude/projects` пуст или Claude Code писал в другой каталог) или панель
ещё не получила первый ответ — тогда достаточно подождать секунду.
...
ручная калибровка, [обновление](#обновление) — состояние и единственная кнопка
«Проверить обновления», — пути к конфигу, кешу и логу с кнопками «Открыть», а
также «Сбросить настройки» — с подтверждением, и сброс не трогает подобранный
бюджет и калибровку: их программа набрала по живым данным.
