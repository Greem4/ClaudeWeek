...
Claude Code различает аккаунты **конфиг-домом** — каталогом, где лежат его
настройки, транскрипты и учётные данные. Обычная установка живёт в `~/.claude`.
Второй аккаунт — это второй такой каталог, и путь к нему передаётся переменной
`CLAUDE_CONFIG_DIR`.
...
```bash
mkdir -p ~/.claude-b
CLAUDE_CONFIG_DIR=~/.claude-b claude    # внутри: /login вторым аккаунтом
```

Проверить, что дома действительно два и вход в один не выбил другой:

```bash
claude auth status                                  # первый аккаунт
CLAUDE_CONFIG_DIR=~/.claude-b claude auth status    # второй
```

Разные адреса в ответах — всё получилось. Обратите внимание: про первый дом
спрашивать нужно **без** `CLAUDE_CONFIG_DIR`. С этой переменной, выставленной
даже в тот же самый путь `~/.claude`, Claude Code отвечает «не вошли» — ячейку
учётных данных выбирает не путь, а сам факт установки переменной. Виджет это
учитывает и переменную для стандартного дома не ставит. Записей в Keychain при этом тоже
станет две: у каждого дома своя, и `/login` во втором не трогает токен первого.

### Что делает виджет

Каталог `~/.claude-b` появился — в заголовке панели встают **два знака
Claude**. Выбранный аккаунт горит фирменным оранжевым, второй гаснет до
приглушённого серого. Знаки одинаковые, потому что сервис один; кто из них
кто, говорит подсказка при наведении — там стоит адрес аккаунта, который
назвал `claude auth status`, а не порядковый номер. Выбор сохраняется в
конфиге и переживает перезапуск.

У каждого аккаунта **своё всё**: свой недельный лимит, свой снимок, свой
подобранный бюджет, своя отсечка счёта и свой журнал уведомлений. Файлы
второго аккаунта лежат рядом с файлами первого под именами с приставкой
`secondary-`. Общими их сделать было нельзя: последнее обновление одного
аккаунта стирало бы снимок другого.

Пока во второй дом не вошли, панель на нём не рисует серые заглушки — это
читалось бы как вечная загрузка. Вместо них стоит строка о том, что входа нет,
и путь, который нужно передать в `CLAUDE_CONFIG_DIR`.

Каталога второго дома нет вовсе — нет и переключателя: программа работает
ровно так же, как с одним аккаунтом, и настраивать для этого ничего не нужно.

### Откуда берётся токен второго аккаунта

Токен каждого конфиг-дома лежит в своей записи Keychain, а имя записи Claude
Code собирает из пути дома: стандартному достаётся `Claude Code-credentials`,
любому другому — то же имя с приставкой из первых восьми hex-символов sha256
пути (`~/.claude-b` → `Claude Code-credentials-034e8c6f`). Имя это нигде не
публикуется, так что ClaudeWeek повторяет формулу — и это осознанная догадка о
чужой внутренней детали: сменится схема именования в CLI, и второй аккаунт
скажет «в этот аккаунт ещё не вошли», пока формулу не поправят. Зато чужих
цифр под своим именем он не покажет: имя выводится из дома, а не из аккаунта,
поэтому промах формулы даёт «записи нет», а не чужой токен.

Раньше запись искали иначе — перебором записей Claude Code со сверкой
`organizationUuid` внутри каждой. Способ умер вместе с полем: на 2.1.251
запись состоит из одного `claudeAiOauth`, организации в ней нет вовсе, и
сверять стало не с чем. Кому принадлежит токен, знает теперь только
`claude auth status`.

По той же причине аккаунт, в который не вошли, не получает «креды по
умолчанию»: иначе он показал бы цифры первого аккаунта под своим именем.

### Путь второго дома

Задаётся ключом `accounts` в `config.json`, по умолчанию `~/.claude-b`:

```json
{
  "accounts": {
    "primaryHome": "~/.claude",
    "secondaryHome": "~/.claude-b"
  }
}
```
...
**По умолчанию панель не разбирает расход по дням вовсе.** Вместо семи
суточных полос — одна, на всю неделю: тот же вид, что у пятичасовой сессии,
голый процент без сравнения с графиком. Панель короче ровно на разбивку по
суткам. Заливка этой полосы, в отличие от суточных, краснеет на критическом
пороге — границе, за которой нет плановой зоны рядом, которая на дейтеранопии
сливалась бы с красным.

Включается разбивка в настройках: вкладка **Общие** → **Расширенные
настройки** → галка **«Дневной план»**. С ней появляются семь полос с синей
плановой зоной и парой чисел «факт / план».

### Вся неделя или только сегодня

Пока дневной план включён, ряд можно свернуть до текущих суток — там же,
пунктом **«Показывать»**: остаются пятичасовая сессия и одна дневная полоса, а
заголовок с часом сброса и футер с итогом недели — на месте. Панель выходит
короче на шесть строк.
...
астрономическим. Рабочий день задаётся в настройках (по умолчанию 10:00 →
18:00); ночью линия плана стоит. Отсюда простое правило: **факт ниже плана —
идёте в графике, выше — сгорите раньше пятницы.**

Ниже — ряд для примера с рабочим днём 11:00 → 00:00 (13 рабочих часов в
сутки, круглое число для наглядного счёта) и окна «ПТ 16:00 → ПТ 16:00»
(московское время — на час меньше). Всего в неделе 91 рабочий час:
...
| Источник данных | `auto` — всегда онлайн, с падением на локальный, `official` — только сервер, `local` — только оценка по транскриптам |
| Обновлять раз в | интервал опроса, 1–30 минут (в сеть — не чаще раза в 60 с) |

Дневной план и всё, что с ним связано, собрано под раскрывающимся пунктом
**«Расширенные настройки»** внизу вкладки — свёрнут по умолчанию, раскрывается
щелчком в любом месте строки заголовка:

| Настройка | Что делает |
|---|---|
| Дневной план | по умолчанию выключен: панель показывает только пятичасовую сессию и итог недели, без разбора по дням (см. [Без дневного плана](#без-дневного-плана)) |
| Показывать | видно только при включённом плане: «Вся неделя» — семь строк, «Только сегодня» — одна, а неделя раскрывается кликом по ней, пока панель открыта |
| Начало недели | видно только при включённом плане: с какого дня идёт ряд суток на панели — «Понедельник» (по умолчанию) или «День сброса». Момент сброса это не двигает — его называет сервер; под настройкой показан тот, что придёт следующим |
| Таймзона | в какой зоне считать границы суток и в какой показывать момент сброса — видно всегда, влияет и на пятичасовую сессию |
| Рабочий день | распорядок из списка («Рабочий день 10–18» — по умолчанию, «Длинный день 10–22», «Вечерний 11–24», «С утра до ночи 9–22», «Круглосуточно») или свои часы степперами; видно всегда — по нему раскладывается и недельный план, и плановая зона в значке строки меню, ночью план стоит. Строка «Получается» показывает выбранное словами: «10:00 – 18:00, 8 ч в сутки» |

«Показывать» и «Начало недели» появляются только при включённом «Дневном
плане» — без разбора по дням делить и переставлять нечего. «Таймзона» и
«Рабочий день» остаются на месте и без него: рабочие часы по-прежнему
двигают план в значке строки меню и прогноз «кончится в …».
...
Сами пороги — под раскрывающимся пунктом **«Расширенные настройки»** внизу
вкладки (свёрнут по умолчанию):
...
| Сообщать о новой версии | баннер, когда на GitHub вышел релиз новее вашего. Не гаснет вместе с общим выключателем: тот про разговоры о расходе, а это новость другого рода |

Пороги обоих лимитов — под раскрывающимся пунктом **«Расширенные настройки»**
внизу вкладки (свёрнут по умолчанию):
...
| Недельный лимит | уведомлять ли о неделе и после скольких процентов — два порога, по умолчанию 80 и 95. В баннере приходит красным числом |
| Пятичасовая сессия | то же для сессии, по умолчанию 75 и 95. В баннере приходит дугой |
...
Короткая версия того, что подробно расписано в
[Авторизация и доступ](#авторизация-и-доступ) в начале этого файла: откуда
берётся токен и что с ним можно и нельзя сделать, плюс кнопка «Проверить
сейчас» — она делает один настоящий запрос и печатает результат: процент или
причину отказа.

Сверху — раздел **«Аккаунты»**: оба конфиг-дома, путь каждого и кто в него
вошёл. Три положения различаются намеренно: «дом не заведён» — каталога нет,
второй аккаунт не заводили; «не вошли» — каталог есть, но `/login` в нём не
делали; адрес — вошли, и видно, кто именно. Советы человеку в первых двух
случаях разные, поэтому одним «нет данных» они не склеиваются. Подробности —
в [Два аккаунта](#два-аккаунта).

Ниже — **«Сейчас в ключе»**: метка аккаунта из записи Keychain, начало UUID
организации и тариф (`7f3a1b2c·max`). Токен обновляется раз в час, а метка
держится — по ней и видно, тот же это аккаунт, что вчера, или вошли другим.
В режиме «только локальная оценка» строка пустая: там Keychain не читается
вовсе, и ради одной подписи нарушать это обещание незачем.

Рядом — **«Начать счёт заново»** (см. [Смена аккаунта](#смена-аккаунта)). Как
работать вовсе без токена и что делать, если macOS снова спросила доступ, —
в [Авторизация и доступ](#авторизация-и-доступ): в самом окне эти два сюжета
были повторением документации, а не настройкой, и переехали сюда целиком.
...
как и в панели), вторая — сколько ждать сброса. Какой это лимит, говорит не
текст, а **картинка справа**: пятичасовая сессия приходит **дугой**,
заполненной израсходованным, недельный лимит — **красным числом**. Это тот же
язык, каким говорит значок в строке меню, где дуга по умолчанию отдана сессии,
а цифра в центре — неделе.

<p align="center">
  <img src="images/notification-week-dark.png" width="440" alt="Баннер недельного лимита: число справа">
  <br>
  <img src="images/notification-session-dark.png" width="440" alt="Баннер пятичасовой сессии: дуга справа">
</p>
...
Окнами разговаривает только нажатая кнопка. Самостоятельная проверка окон не
открывает: нашла — строка внизу панели и баннер macOS, не дозвонилась до
GitHub — запись в лог, и ничего поверх работы.

О версии баннер говорит **один раз**, а не каждые сутки до установки: сказанное
запоминается в `alerts.json` и переживает перезапуск. Кнопки в нём нет
намеренно — установка перезаписывает приложение и перезапускает его, а такому
не место за одним щелчком из-под чужой работы. Выключается на вкладке
«Уведомления», отдельно от разговоров о лимитах.
...
|---|---|---|
| `language` | `system` | язык интерфейса: `system`, `ru` или `en`. Тот же выбор, что на вкладке «Общие» |
...
| `refreshInterval` | `300` | секунды между обновлениями. Меньше 30 чинится на 30, ползунок в настройках начинается с 60. В сеть при этом ходят не чаще раза в минуту в любом случае — короткий интервал только чаще перерисовывает панель по уже полученному числу |
| `provider` | `auto` | `official`, `local` или `auto` |
| `workHours.start` / `.end` | `10` / `18` | рабочий день, по которому раскладывается лимит; `24` — полночь. `0`/`24` — считать круглосуточно, как раньше |
...
| `calibration.observedPercent` / `.at` | — | ручная калибровка: официальный процент и момент, когда он наблюдался. Программа подбирает бюджет и сама, складывая его в `cache.json`, — заданное здесь важнее |
...
| `notifications.update` | `true` | баннер о вышедшей версии |
...
| `appearance.showsPlan` | `false` | `true` включает разбор по дням: семь суточных полос с планом вместо одной полосы недели; без него `panelLayout` не смотрят |
...
- **Выбор аккаунта** — флаг `--account=primary|secondary` выбирает, чей лимит
  показать, в том числе для `--json`. Без флага берётся тот аккаунт, что
  выбран в панели.
...
Вошли в Claude Code другим аккаунтом — рабочим вместо домашнего, — и виджет
показывает две правды сразу. Недельный процент приходит от сервера и сменяется
сам: сервер узнаёт аккаунт по токену. А вот разбивка по суткам считается по
транскриптам в `projects/` того же конфиг-дома, и они пишутся в одни и те же
файлы, кто бы ни был залогинен в этот дом. Различить их по содержимому нечем — маркера аккаунта в
записях нет. Без отсечки расход прежнего аккаунта считался бы в новый лимит до
конца недели.

Поэтому у счёта есть **начало**. Программа замечает смену сама: сверяет метку
аккаунта из Keychain (начало UUID организации и тариф) с той, на которой ведётся
счёт, и при расхождении начинает заново. Метка живёт в
`~/.config/claude-week/state.json` вместе с моментом отсечки.

Первый запуск сбросом не считается: метки в состоянии ещё нет, сравнивать не с
чем, и программа просто запоминает, с кем имеет дело. По той же причине не
сработает автосброс сразу после обновления с версии, которая метку не писала, —
там счёт сбрасывается кнопкой.

Кнопка эта — **«Начать счёт заново»** на вкладке «Доступ». Нужна и тогда, когда
Keychain недоступен: без него метку не прочитать, и смену аккаунта заметить
неоткуда. Что делает:

- ставит отсечку по текущей минуте — расход до неё в счёт больше не идёт;
- стирает `cache.json`: в нём процент, подобранный бюджет и сессия прежнего
  аккаунта;
- стирает `alerts.json`: о порогах прежнего аккаунта уже сказали, и повторять
  это, считая с нуля, — шум.

Настройки, транскрипты и недельный процент от сервера остаются на месте. Индекс
транскриптов тоже не трогается: старые записи из него отсекает та же отсечка, а
выброшенный он стоил бы полного перечитывания `~/.claude/projects` на ровном
месте.

Пока официальный источник отвечает, разница видна только в суточных полосах:
итог недели сервер и так считает по нынешнему аккаунту. Офлайн отсечка важнее —
локальная оценка целиком построена на транскриптах.

Сами полосы после сброса не пустеют. Расход до отсечки сервер уже насчитал, и
показать его надо — но подённой правды о нём нет: тем и занята отсечка, чтобы
прежний счёт не смешивался с нынешним. Поэтому он раскладывается по дням тем же
ровным темпом, каким считается план, — от начала недели до момента сброса. Ряд
идёт вдоль плановой линии и приходит к общему проценту ровно в сутках сброса, а
всё, что потрачено после, добавляется к нему своими сутками. Горкой в один день
этот расход не сваливается намеренно: в тех сутках зажигался бы перерасход,
которого не было.
