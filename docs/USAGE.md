...
Ничего вводить не нужно. Для **Codex** приложение запускает установленный
`codex app-server`: официальный процесс сам использует вход Codex CLI или
Codex app и возвращает только лимиты и токенную статистику. ClaudeWeek не
читает и не хранит токены Codex. Если команда `codex` не найдена или в клиенте
не выполнен вход, вкладка Codex честно остаётся без данных.

Для **Claude** приложение берёт OAuth-токен уже авторизованного
...
Переключатель в заголовке выбирает Claude или Codex: два знака самих
сервисов рядом с надписью, без рамок и подложек. Выбранный горит своим цветом
— у Claude фирменным оранжевым, — невыбранный приглушён серым. Кнопки не выполняют вход и не меняют аккаунт внутри клиентов — только
показывают лимиты уже авторизованного сервиса. Последний выбор запоминается. У каждого сервиса свой
снимок, поэтому при переключении цифры одного никогда не подписываются именем
другого даже на время обновления.
...
Сверху — **«Сейчас в ключе»**: метка аккаунта из записи Keychain, начало UUID
...
| `activeAccount` | `claude` | какой сервис выбран в панели: `claude` или `codex` |
...
- **Codex** — официальный `codex app-server`: `account/rateLimits/read`
  сообщает короткое и недельное окна с процентами и сбросами, а
  `account/usage/read` — дневные токенные бакеты. Процент недели точный;
  дневные полосы — форма, масштабированная к нему. Схема — в [API.md](API.md).
- **Официальный источник Claude** — тот же процент, что в `/usage`: `GET
...
- **Короткое окно** — отдельная полоса над сутками с часом сброса. У Claude
  оно пятичасовое, у Codex длительность приходит в `windowDurationMins` и
  поэтому подпись может быть другой. Число официальное; локально оно не
  считается, поэтому для Claude офлайн показывается последнее
...
Сначала не путайте два действия: кнопки в панели переключают **Claude и
Codex**, а вход другим аккаунтом выполняется в самом Claude Code или Codex.
ClaudeWeek всегда следует за входом соответствующего клиента.
...
транскриптам в `~/.claude/projects`, и они пишутся в одни и те же файлы, кто бы
ни был залогинен. Различить их по содержимому нечем — маркера аккаунта в
...
ClaudeWeek --account=X     сервис для UI и --json: claude или codex
...
Незнакомый флаг, неизвестный сервис у `--account=`, неизвестный источник у
`--provider=` или не-число у `--calibrate=` — это ошибка с кодом 2 и
подсказкой, а не молчаливый запуск иконки в строке меню.
