...
// MARK: - Язык в окружении

private struct StringsKey: EnvironmentKey {
    /// Русский по умолчанию — тот язык, на котором панель говорила до
    /// появления второго. Живое дерево всегда получает язык из конфига;
    /// значение отсюда достаётся только предпросмотрам.
    static let defaultValue = L10n(Lang.ru)
}

extension EnvironmentValues {
    /// Строки интерфейса на всё дерево разом. Панель и окно настроек ставят
    /// их один раз, поэтому строки дней, сессии и моделей не таскают язык
    /// параметром через полдюжины инициализаторов — а смена языка в конфиге
    /// перерисовывает их тем же движением, что и всё остальное.
    var strings: L10n {
