...
func runWeekWindowTests(_ t: Harness) {
    ...
    // Сброс среди ночи: огрызок последних суток лежит вне рабочих часов и
    // плана не набирает вовсе. Пока он вставал в ряд, две нижние строки
    // показывали по 100 % — одну и ту же сотню дважды.
    t.suite("окно недели: сброс среди ночи") {
        // Вторник (weekday 3), сброс в 02:00, рабочий день 11–24.
        let window = WeekWindow(containing: at(2026, 8, 12, 12, 0), config: config(weekday: 3, hour: 2))
        t.equal(window.start, at(2026, 8, 11, 2, 0), "старт — вторник 02:00")
        t.check(window.splitsResetDay, "вторник разрезан надвое")

        let labels: (Date) -> [String] = { moment in
            window.rows(at: moment).map { Formatting.weekdayShort($0.start, calendar: window.calendar) }
        }
        t.equal(labels(at(2026, 8, 18, 0, 30)), ["ВТ", "СР", "ЧТ", "ПТ", "СБ", "ВС", "ПН"],
                "после полуночи ряд не переезжает на ночной огрызок")
        t.equal(window.rows(at: at(2026, 8, 18, 0, 30)).last?.index, 6,
                "последняя строка — понедельник, а не два часа перед сбросом")
        t.equal(window.highlightedRow(at: at(2026, 8, 18, 0, 30)), 0,
                "сегодняшний вторник подсвечен своей первой половиной")

        let plan = window.rows(at: at(2026, 8, 18, 0, 30)).map { Int($0.planPercent.rounded()) }
        t.equal(plan, [14, 29, 43, 57, 71, 86, 100], "ряд приходит к 100 % ровно один раз")

        // Круглые сутки — огрызок свой вес имеет, и правило его не трогает.
        let allDay = WeekWindow(
            containing: at(2026, 8, 12, 12, 0),
            config: config(weekday: 3, hour: 2, work: WorkHours(start: 0, end: 24))
        )
        t.equal(allDay.rows(at: at(2026, 8, 18, 0, 30)).last?.index, 7,
                "при круглосуточном плане последние сутки в ряду остаются")
    }
