...
func runWeekWindowTests(_ t: Harness) {
    ...
    t.suite("окно недели: начало в понедельник") {
        ...
        // Дню сброса достаётся утренняя половина: с ней ряд от понедельника
        // до пятницы читается сверху вниз и приходит ровно к 100 %.
        let plan = window.rows(at: at(2026, 8, 9, 12, 0)).map { Int($0.planPercent.rounded()) }
        t.equal(plan, [53, 67, 81, 96, 100, 24, 38], "план доходит до 100 % в строке дня сброса")
