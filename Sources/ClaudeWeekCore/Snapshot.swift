...
public struct SessionUsage: Codable, Sendable, Equatable {
    ...
    /// Длина окна: лимит и зовётся пятичасовым, и приходит из поля `five_hour`.
    /// Отдельного начала сервер не присылает, поэтому считаем его от сброса —
    /// другого способа сказать «сессия с 17:30» у нас нет.
    public static let length: TimeInterval = 5 * 3600

    /// Начало пятичасового окна.
    public var startedAt: Date { resetsAt.addingTimeInterval(-SessionUsage.length) }
