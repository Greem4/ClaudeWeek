...
План — это ровный расход недельного лимита по времени: 100 % растягиваются на
168 часов окна и приходят ровно к моменту сброса. Отсюда все цифры в колонке
справа, и отсюда же простое правило: **факт ниже плана — идёте в графике,
выше — сгорите раньше пятницы.**

Ряд плана для окна «ПТ 16:00 → ПТ 16:00» (московское время — на час меньше):

| Строка | Часы в окне | План к концу строки |
|---|---|---|
| ПТ (вечер) | 16:00 → 0:00 | 5 % |
| СБ | сутки | 19 % |
| ВС | сутки | 33 % |
| ПН | сутки | 48 % |
| ВТ | сутки | 62 % |
| СР | сутки | 76 % |
| ЧТ | сутки | 90 % |
| ПТ (утро) | 0:00 → 16:00 | 100 % |

Настройка «План считать на» сдвигает опорную точку строки с конца суток на их
середину — ряд становится 2/12/26/40/55/69/83/95, а смысл не меняется.

Что из этого следует на практике:

- **99 % в пятницу к 14:00 — это ровно по плану**, а не перерасход: за два
  часа до сброса линия плана стоит на 98,8 %. Остаток догорит сам.
- **Пятничное утро — полноценные рабочие часы, а не хвост недели.** До сброса
  в 16:00 план отводит на них 9,5 % лимита; не потратите — они сгорят вместе
  с окном, перенести их на следующую неделю нельзя.
- **Вечер пятницы после сброса — уже новая неделя.** На него плана всего 5 %:
  начали работать вечером и выбрали 15 % — вы не «в начале недели», вы уже
  обгоняете график втрое.
- **Ночь план тоже двигает.** Линия растёт круглосуточно, поэтому за ночь
  отставание само отыгрывает около 6 %: утренние 3–4 % перерасхода к обеду
  обычно рассасываются без всяких мер.
- **Темп в футере — то же самое одним числом.** `1.0×` — вровень, `1.4×` —
  при таком темпе лимит кончится в четверг, и вторая строка футера скажет,
  во сколько именно.
...
| План считать на | середину суток (2/12/26/40…) или их конец (5/19/33/48…) |
...
Данные бывают старыми, и панель об этом говорит: если снимку больше двух минут,
под заголовком появляется «данные 6 мин назад». Появиться этой строке есть от
чего — после отказа сервера виджет держит паузу (1 → 2 → 4 → 8 → 15 минут) и всё
это время показывает последнее удачное число.
...
которая нужна офлайн. План считается на середину суток, опорная точка
