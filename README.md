...
| `appearance.showSessionLabel` | `true` | слово «сессия» в подписи под её полосой; без него остаётся один час сброса |
...
| Кружок | Что значит |
|---|---|
| зелёный, залитый | свежий ответ сервера — то же число, что в `/usage` |
| жёлтый, залитый | цифры сервера, но из кеша: сеть или авторизация отвалились |
| красный, контурный | локальная оценка по транскриптам |
| серый, контурный | данных нет вовсе — первый запуск или отказ без кеша |
...
скажет словами, что именно случилось: «нет сети», «данные 20 мин назад»,
...
он желтеет, а подсказка называет возраст — «данные 6 мин назад». Пожелтеть ему
есть от чего — после отказа сервера виджет держит паузу (1 → 2 → 4 → 8 → 15
минут) и всё это время показывает последнее удачное число.
